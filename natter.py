...
def natter_main(show_title = True):
    ...
    if notify_sh:
        ...
        ips = [f"{int(x):02x}" for x in outer_ip.split(".")]
        ip4p = f"2001::{int(outer_port):x}:{ips[0]}{ips[1]}:{ips[2]}{ips[3]}"
        Logger.info("Calling script: %s" % notify_sh)
        subprocess.call([
            os.path.abspath(notify_sh), protocol, str(inner_ip), str(inner_port), str(outer_ip), str(outer_port), ip4p
        ], shell=False)
